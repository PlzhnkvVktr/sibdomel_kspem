    ...
    Column(
        ...
        Column (
            ...
            Row (modifier = Modifier.fillMaxWidth().padding(bottom = 20.dp), horizontalArrangement = Arrangement.Center) {
                Text(text = TestEnum.nameMV.testName, style = MaterialTheme.typography.h5)
            }
            ...
            Row() {
                ...
                TableCell(text = viewModel.testItem.before_u_uv_mv.value)
                TableCell(text = viewModel.testItem.before_u_vw_mv.value)
                TableCell(text = viewModel.testItem.before_u_wu_mv.value)
                TableCell(text = viewModel.testItem.before_i_u_mv.value)
                TableCell(text = viewModel.testItem.before_i_v_mv.value)
                TableCell(text = viewModel.testItem.before_i_w_mv.value)
            }
            Row(Modifier.background(Color.Gray)) {
                TableCell(text = "Наименование")
            ...
            Row() {
                TableCell(text = viewModel.testItem.name.value)
                TableCell(text = viewModel.testItem.after_u_uv_mv.value)
                TableCell(text = viewModel.testItem.after_u_vw_mv.value)
                TableCell(text = viewModel.testItem.after_u_wu_mv.value)
                TableCell(text = viewModel.testItem.after_i_u_mv.value)
                TableCell(text = viewModel.testItem.after_i_v_mv.value)
                TableCell(text = viewModel.testItem.after_i_w_mv.value)
            }
